...
// Login Handler (Fixed to use backend 'auth:login' event)[cite: 1]
...
// Signup Handler (Fixed to use backend 'auth:signup' event)[cite: 1]
...
// Auth Success listener matching server event[cite: 1]
  ...
  if (room === currentRoom) {
    socket.emit('chat:fetch_history', { room: currentRoom }, (messages) => {
      socket.emit('poll:fetch', currentRoom, (polls) => {
        const container = document.getElementById('messages-container');
        container.innerHTML = '';
        // Combine and render messages and polls chronologically if needed, or render polls through chat stream
        messages.forEach(msg => renderMessage(msg));
        polls.forEach(poll => renderMessage({ ...poll, type: 'poll', sender: poll.creator }));
      });
    });
  }
});
...
// Assets Management with Delete support[cite: 1]
